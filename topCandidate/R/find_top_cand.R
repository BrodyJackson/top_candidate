...
#' @keywords top candidate
#' @export
#' @examples
#' find_top_cand(pine, spruce, cleaned_ortho, binomial_cuts)

find_top_cand <- function(species_one_data, species_two_data, orthologs, the_quantiles, binomial_cuts){
  ...
  for (i in 1:length (the_quantiles)){
    ...
    sub_good_pine <- merge (sub_good_pine,ortho,by.x = "names.snps_count2.",by.y = "pine", all.x = T) #many of the rows do not have an ortholog identified in the other species, so there is an "NA" in the 4th column
    ...
    for (j in 1:length (binomial_cuts)){
      ...
      num_pin[i,j] <- sum (sub_pin$names.snps_count2. %in% orthologs$pine) #how many genes are top candidates in pine?
      num_spr[i,j] <- sum (sub_spr$names.snps_count2. %in% orthologs$spruce) #how many genes are top candidates in spruce?
